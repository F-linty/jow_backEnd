...
export class MaterialCategoriesService {
  ...
  private assertParentNotInSubtree = async (nodeId: number, parentId: number) => {
    ...
    while (cursor !== null) {
      if (cursor === nodeId) {
        throw new HttpException('父级物料分类不能是当前分类的下级', 400);
      }
      if (visited.has(cursor)) {
        throw new HttpException('物料分类层级数据异常，请联系管理员', 400);
      }
      visited.add(cursor);
      const node = await this.prisma.materialCategory.findFirst({
  ...
  async create(body: { name?: string; parentId?: number | string | null; sort?: number }) {
    ...
    if (!name) throw new HttpException('物料分类名称不能为空', 400);
    ...
    if (normalizedParentId !== null) {
      const parent = await this.prisma.materialCategory.findFirst({
        where: { id: normalizedParentId, deletedAt: null },
        select: { id: true },
      });
      if (!parent) throw new HttpException('父级物料分类不存在', 404);
    }
  ...
  async update(
    ...
    if (!existing) throw new HttpException('物料分类不存在', 404);
    ...
    if (normalizedParentId !== null) {
      ...
      if (parentId === id) throw new HttpException('父级物料分类不能是自身', 400);
      const parent = await this.prisma.materialCategory.findFirst({
    ...
    if (body.name !== undefined) {
      ...
      if (!n) throw new HttpException('物料分类名称不能为空', 400);
      payload.name = n;
    }
  ...
  async disable(id: number) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (!existing) {
        throw new HttpException('物料分类不存在', 404);
      }

      const rows = await tx.materialCategory.findMany({
      ...
      return tx.materialCategory.findUnique({ where: { id } });
    });
  }
  ...
  async enable(id: number) {
    ...
    if (existing.parentId) {
      ...
      if (!parent) {
        throw new HttpException('父级物料分类不存在，无法启用', 400);
      }
      if (parent.status !== RecordStatus.ACTIVE) {
        throw new HttpException('请先启用父级物料分类', 400);
      }
    }
  ...
  async remove(id: number) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        return tx.materialCategory.findFirst({ where: { id } });
      });
    } catch (error) {
